describe('todo functional tests', () => {
  ...
  it('completes a todo', () => {
    // arrange
    const title = 'A title at ' + new Date().getSeconds() + new Date().getMilliseconds();
    cy.get('input[id="txtTodoItemToAdd"]').type(title);
    cy.get('[id="btnAddTodo"]').click()

    // act
    cy.get('.todo--toggle-completed').click()
    cy.get('[id="todoList"]').children().first().should('have.class', 'todo--completed');
    
    cy.get('.todo--toggle-completed').click()
    cy.get('[id="todoList"]').children().first().should('not.have.class', 'todo--completed');
  });
});
